...
Kinect::Kinect(){
colorStreamHandle = CreateEventW(NULL,TRUE,FALSE,NULL);
...
void Kinect::getSkeleton(float* skeletonX,float* skeletonY){
	if (SUCCEEDED(NuiSkeletonGetNextFrame(0, &skeletonFrame)))
        {	
			for (int i = 0; i < NUI_SKELETON_COUNT ; i++)
				{
					...
					switch (skeleton.eTrackingState)
						{
							...
							/*default:
								for(int i=0;i<21;i++){
									ofLog(OF_LOG_NOTICE,"No skeleton");
									skeletonX[i]=-1;
									skeletonY[i]=-1;
								}
								break;*/
